...
# }
mass.spectra <- hyperSpec2MALDI(hyperspecobj,cellname = cell.names,rawdata = raw.data)
...
cell.name=rep("",length(cell.names))
for (i in 1:length(cell.names)) {
  ...
  mq <- list()
for (i in 1:length(cell.names)){
  mass.spectrum <- createMassSpectrum(mass=hyperspecobj@wavelength,
                                      intensity=raw.data[i,])
...
iterationsplot(mq.trim[[1]],iteration.options)
# optimal number of iterations?
number.of.iterations <- 10
i <- 2
iterationsplot(mq.trim[[i]],number.of.iterations)
...
# correct all spectra
mass.spectra.baseline.corr <- removeBaseline(mq.trim,
                                             method="SNIP",
